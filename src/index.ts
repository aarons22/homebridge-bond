      ...
      case DeviceType.CeilingFan:
        {
          ...
          if (Device.CFhasLightbulb(device)) {
            ...
            let dimmerUp = accessory.getService(`${device.location} ${device.name} DimmerUp`);
            let dimmerDown = accessory.getService(`${device.location} ${device.name} DimmerDown`);
            if (this.config.include_dimmer) {
              ...
              if (Device.HasSeparateDimmers(device)) {
                if (dimmerUp === undefined) {
                  dimmerUp = accessory.addService(new hap.Service.Switch(`${device.location} ${device.name} DimmerUp`, "up"));
                }
                if (dimmerDown === undefined) {
                  dimmerDown = accessory.addService(new hap.Service.Switch(`${device.location} ${device.name} DimmerDown`, "down"));
                }
